import sys
import os
sys.path.append(os.path.abspath(os.path.dirname(__file__)))


LABEL_target_feature_analysis = 'target_feature'
LABEL_approximation_parameters = 'approx_params'
LABEL_info_task = 'info_task'
LABEL_ciq_scores = 'scores'
LABEL_dataframe_approximated = 'new_df'
LABEL_discrete_intervals = 'discrete_intervals'
LABEL_grouped_features = 'grouped_features'

LABEL_target_value = 'target'
LABEL_predicted_value = 'pred'

LABEL_binary_metrics = 'binary'
LABEL_distance_metrics = 'distance'
LABEL_causal_graph_distance_metrics = 'causal_graph_distance'
LABEL_causal_graph_similarity_metrics = 'causal_graph_similarity'

LABEL_dir_storing_dict_and_info = 'results_sensitive_analysis'

LABEL_kind_group_var = 'kind'
LABEL_value_group_var = 'value'

LABEL_reward_action_values = 'reward_action_values'


